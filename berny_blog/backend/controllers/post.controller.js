...
export const createPost = async (req, res) => {
  try {
    ...
    res.status(201).json(post); // Changed to 201 for created resource
  } catch (error) {
...
export const uploadAuth = async (req, res) => {
  try {
    // Initialize ImageKit inside the function to ensure environment variables are available
    ...
    res.status(500).json({ error: "Failed to get upload auth" });
  }
};